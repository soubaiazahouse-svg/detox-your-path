...
export default function SplashScreen({ navigation }: Props) {
  ...
  const handleStart = async () => {
    ...
    navigation.replace(user?.onboarded ? 'Main' : 'Onboarding');
  };
      ...
      <Animated.View style={[styles.content, { opacity: fadeAnim, transform: [{ scale: scaleAnim }] }]}>
        ...
        <TouchableOpacity style={styles.btn} onPress={handleStart} activeOpacity={0.85}>
          <LinearGradient colors={gradients.sky} style={styles.btnGrad}>
            <Text style={styles.btnText}>Get Started — ابدأ الآن</Text>
          </LinearGradient>
        </TouchableOpacity>
      </Animated.View>
    </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  btn: { width: '100%', borderRadius: 50, overflow: 'hidden', shadowColor: colors.sky, shadowOpacity: 0.5, shadowRadius: 20, elevation: 8 },
